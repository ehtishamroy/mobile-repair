...
@section('content')
      ...
      <div class="row mb-custom">
        ...
        <div class="col-md-6">
          ...
          <a href="{{ route('frontend.contact') }}"
            class="btn btn-gradient text-decoration-none">{{ $content->what_we_offer_button_text ?? 'Contact Us' }}</a>
        </div>
      </div>
      <div class="row row-cols-lg-3 row-cols-md-2 row-cols-1 g-3">
        @php
          $services = \App\Models\RepairService::where('is_active', true)->orderBy('order')->take(6)->get();

          // Fallback if no services in DB, though ideally we should have them
          if ($services->isEmpty()) {
            $defaultServices = [
        ...
        @foreach($services as $service)
          <div class="col">
            @if($service instanceof \App\Models\RepairService)
            ...
            @else
              {{-- Fallback for array data if DB is empty --}}
              <a href="{{ route('frontend.book-repair') }}" class="text-decoration-none d-block h-100">
                <div class="repair-service-card h-100">
                  ...
                  <h3 class="fs-24">{{ $service['name'] }}</h3>
                  <p class="fs-16">
                    Lorem Ipsum is simply dummy Lorem Ipsum is simply dummy text
                  </p>
                </div>
              </a>
            @endif
      ...
      <div class="container">
        ...
        <div class="flex-between flex-wrap flex-md-nowrap mb-5">
          ...
          @if($trendingBrands->count() > 0)
            <ul class="nav nav-pills justify-content-between justify-content-md-end mb-3 gap-3 mt-md-0 mt-3" id="pills-tab"
              role="tablist">
              @foreach($trendingBrands as $index => $brand)
                <li class="nav-item" role="presentation">
                  <a href="{{ route('frontend.marketplace', ['category' => $smartphoneCategory->id, 'brand' => $brand->id]) }}"
                  ...
                  <div class="card-body">
                    ...
                    <div class="product-price {{ $product->compare_at_price ? 'text-promo' : '' }}">
                      ...
                      @if($product->compare_at_price)
                        <small
                          class="text-muted text-decoration-line-through">{{ $currencySymbol }}{{ number_format($product->compare_at_price, 2) }}</small>
                      @endif
                    </div>
                  </div>
                </div>
  ...
  <section class="quality-repair-section mb-custom py-5 py-sm-0">
    <div class="container d-flex flex-column align-items-center text-center gap-lg-5 gap-4">
      <span
        class="qm-badge text-uppercase">{{ $content->quality_repairs_badge ?? 'Harrow Mobile & Laptop at Glance' }}</span>
      ...
      <div class="container">
        ...
        <div class="flex-between flex-wrap flex-md-nowrap mb-5">
          ...
          @if(count($latestProductsData) > 1)
            <ul class="nav nav-pills justify-content-between justify-content-md-end mb-3 gap-3 mt-md-0 mt-3"
              id="pills-latest-products-tab" role="tablist">
              @foreach($latestProductsData as $categoryId => $data)
                <li class="nav-item" role="presentation">
                  <button class="nav-link {{ $loop->first ? 'active' : '' }}" id="pills-category-{{ $categoryId }}-tab"
                    data-bs-toggle="pill" data-bs-target="#pills-category-{{ $categoryId }}" type="button" role="tab"
                    aria-controls="pills-category-{{ $categoryId }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}">
                    {{ $data['category']->name }}
                  </button>
                </li>
              @endforeach
        ...
        <div class="tab-content" id="pills-latest-products-tabContent">
          ...
          @foreach($latestProductsData as $categoryId => $data)
            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="pills-category-{{ $categoryId }}"
              role="tabpanel" aria-labelledby="pills-category-{{ $categoryId }}-tab">
              <div class="row row-cols-xxl-5 row-cols-xl-4 row-cols-lg-3 row-cols-sm-2 row-cols-1 g-3">
