                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                name="name" value="{{ $nameValue }}" required>
                            @error('name')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('repair_brand_id') is-invalid @enderror" id="repair_brand_id"
                                ...
                                @foreach($brands as $brand)
                                    <option value="{{ $brand->id }}" {{ old('repair_brand_id', $deviceType->repair_brand_id) == $brand->id ? 'selected' : '' }}>
                                        {{ $brand->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('order') is-invalid @enderror" id="order"
                                name="order" value="{{ $orderValue }}" min="0">
                            @error('order')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" id="is_active" name="is_active"
                                    value="1" @if($isActive) checked @endif>
                ...
                <div class="card card-primary">
                    ...
                    <div class="card-body">
                        @if($deviceType->featured_image)
                            <div class="mb-3">
                                <img src="{{ asset('storage/' . $deviceType->featured_image) }}" alt="{{ $deviceType->name }}"
                                    class="img-fluid" style="max-height: 200px;">
                                <p class="text-muted small mt-1">Current featured image</p>
                            </div>
                        @endif
                        <div class="form-group">
                            <label for="featured_image">{{ $deviceType->featured_image ? 'Change' : 'Upload' }} Device
                                Image</label>
